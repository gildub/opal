...
const resolvers = {
    Query: {
        ...
        folder: (_, { provider, id }, { dataSources }) => dataSources.inventoryAPI.getFolder(provider, id),
        providers: (_, { vsphere }, { dataSources }) => dataSources.inventoryAPI.getProviders(vsphere),
        provider: (_, { vsphere, name }, { dataSources }) => dataSources.inventoryAPI.getProvider(vsphere, name),
        datacenters: (_, { provider }, { dataSources }) => dataSources.inventoryAPI.getDatacenters(provider),
        datacenter: (_, { provider, id }, { dataSources }) => dataSources.inventoryAPI.getDatacenter(provider, id),
        clusters: (_, { provider }, { dataSources }) => dataSources.inventoryAPI.getClusters(provider),
        cluster: (_, { provider, id }, { dataSources }) => dataSources.inventoryAPI.getCluster(provider, id),
        datastores: (_, { provider }, { dataSources }) => dataSources.inventoryAPI.getDatastores(provider),
        datastore: (_, { provider, id }, { dataSources }) => dataSources.inventoryAPI.getDatastore(provider, id),
        ...
        networks: (_, { provider, filter }, { dataSources }) => dataSources.inventoryAPI.getNetworks(provider, filter),
        network: (_, { provider, id }, { dataSources }) => dataSources.inventoryAPI.getNetwork(provider, id),
        vm: (_, { provider, id }, { dataSources }) => dataSources.inventoryAPI.getVM(provider, id),
        vms: (_, { provider, filter }, { dataSources }) => dataSources.inventoryAPI.getVMs(provider, filter),
    },
    FolderGroup: {
        __resolveType(obj) {
            if (obj.kind === 'Folder')
    ...
    Folder: {
        children: async (folder, _, { dataSources }) => {
            const children = [];
            Promise.all(folder.children.map((child) => {
